extends Node3D
## ============================================================================
## MetaHuman -> Godot  ·  COMPARISON-SPEC capture tool (forked from release.gd)
## ============================================================================
## Everything below is an UNMODIFIED copy of scenes/release.gd (the shipped,
## public look-dev/release tool — see that file's own header for what IT does).
## This fork exists so a one-off research capture (the cross-platform
## metahuman-consistency comparison, see
## knowledge/projects/metahuman-consistency/COMPARISON-SPEC.md) does not touch
## the public tool's script. It reuses release.gd's PROVEN character-loading,
## material-wiring (skin_shader_explainer SSS, eye.gdshader, hair card shaders),
## and blendshape machinery verbatim — only a NEW, additive entrypoint is added
## (see "COMPARISON-SPEC CAPTURE MODE" below, search for _run_comparison_mode).
## Nothing in the original release.gd behavior is changed; COMPARE_CAPTURE is a
## new env var, independent of RELEASE_CAPTURE, checked first in _ready().
##
## COMPARISON-SPEC CAPTURE MODE (new):
##   COMPARE_CAPTURE=1 COMPARE_CHAR=her|guy COMPARE_OUT=<abs dir> [COMPARE_YAW=0]
##   Renders the canonical C1-C7 camera x E0-E5 expression set from
##   COMPARISON-SPEC.md, in the SAME shading pipeline release.gd ships (full
##   skin/eye/hair shaders, AgX tonemap, SSIL), but with the spec's NEUTRAL
##   studio lighting/backdrop instead of release.gd's mood rig, and camera
##   transforms computed from the spec's head-joint-relative table instead of
##   the interactive orbit or the fixed UE-matched cam. Writes
##   <COMPARE_OUT>/<camera>_<expression>.png (1024x1024 by launch --resolution)
##   + <COMPARE_OUT>/manifest.json, then quits.
##
## Original release.gd docstring (still describes everything below unchanged):
...
## ---- COMPARISON-SPEC tables (new) -------------------------------------------
## From knowledge/projects/metahuman-consistency/COMPARISON-SPEC.md. Units cm,
## camera "pos" is relative to the head joint, right-handed Z-up, subject faces
## -Y (spec's own convention). See _spec_vec() for the derivation of the
## Godot-space conversion (a PURE ROTATION, both conventions are right-handed —
## unlike the UE conversion elsewhere in this file, no reflection is needed).
## "origin" (new, not in the spec table): which anchor point the spec's "pos" offset
## is measured from. Defaults to "head" (the literal reading of the table header,
## "Camera pos (rel. head joint)") EXCEPT for C3/C4, set to their own lookat target.
## Why: this MetaHuman skeleton's "head" bone sits well back/below the eyes (measured
## ~1.55m vs a ~1.65-1.70m crown on a 1.74m figure) -- fine for the wide shots (C1's
## 55cm/28deg frame is tens of cm tall, absorbs it) but NOT for these two: at 18cm/
## 18deg the vertical frame is only ~5.7cm, so "18cm forward of head_w" over/under-shoots
## the actual eye/mouth surface enough to clip into geometry (verified: rendering with
## origin="head" produced blurred skin, not a clean eye/mouth macro). A real macro photo
## of an eye is 18cm from the EYE, not from an interior skull point -- this is the more
## physically faithful reading of "18cm eye-macro camera", disclosed as a deviation in
## the manifest, not a silent substitution of a different camera.
const SPEC_CAMERAS := [
	{"id": "C1_portrait",   "pos": [0.0, -55.0, 0.0],   "lookat": "head",    "fov": 28.0, "origin": "head"},
	{"id": "C2_profile_L",  "pos": [-55.0, 0.0, 0.0],   "lookat": "head",    "fov": 28.0, "origin": "head"},
	{"id": "C3_eye_macro",  "pos": [0.0, -18.0, 2.0],   "lookat": "eye_mid", "fov": 18.0, "origin": "eye_mid"},
	{"id": "C4_mouth_macro","pos": [0.0, -18.0, -4.0],  "lookat": "mouth",   "fov": 18.0, "origin": "mouth"},
	{"id": "C5_crown",      "pos": [0.0, -20.0, 22.0],  "lookat": "head",    "fov": 30.0, "origin": "head"},
	{"id": "C6_bust_seam",  "pos": [0.0, -45.0, -12.0], "lookat": "neck_01", "fov": 30.0, "origin": "head"},
	{"id": "C7_full_body",  "pos": [0.0, -260.0, -80.0],"lookat": "pelvis",  "fov": 34.0, "origin": "head"},
]
const SPEC_EXPRESSIONS := [
	{"id": "E0_neutral",  "shapes": {}},
	{"id": "E1_jawOpen",  "shapes": {"jawOpen": 1.0}},
	{"id": "E2_smile",    "shapes": {"mouthSmileLeft": 1.0, "mouthSmileRight": 1.0}},
	{"id": "E3_browsUp",  "shapes": {"browInnerUp": 1.0, "browOuterUpLeft": 1.0, "browOuterUpRight": 1.0}},
	{"id": "E4_blink",    "shapes": {"eyeBlinkLeft": 1.0, "eyeBlinkRight": 1.0}},
	{"id": "E5_lookLeft", "shapes": {"eyeLookOutLeft": 1.0, "eyeLookInRight": 1.0}},
]
...
func _ready() -> void:
	# NEW: COMPARISON-SPEC capture mode is a completely separate init path — it
	# does not fall through to any of release.gd's original _ready() body below
	# (mood lighting, UI, orbit camera), so the original tool's behavior is
	# provably unaffected by this fork when COMPARE_CAPTURE is unset.
	if OS.has_environment("COMPARE_CAPTURE"):
...
func _process(delta: float) -> void:
	# NEW: COMPARISON-SPEC capture mode drives the camera/blendshapes/eye-bones
	# explicitly and synchronously inside _run_comparison_mode()'s own loop; none
	# of release.gd's per-frame interactive behavior below (orbit camera, turntable,
	# leader-pose, idle sway, hero cam...) should run during it, or it stomps the
	# spec camera transform every frame (found empirically: the orbit camera's
	# default h_offset/yaw/pitch/fov was overwriting the intended C1-C7 framing
	# because _update_orbit_camera()'s guard only checked RELEASE_CAPTURE, not this
	# mode's COMPARE_CAPTURE). Character stays exactly as _load_character() +
	# the explicit blendshape/eye-bone calls in _run_comparison_mode() leave it.
	if OS.has_environment("COMPARE_CAPTURE"):
		return
...
## ============================================================================
## COMPARISON-SPEC CAPTURE MODE (new, additive — nothing above this line was
## changed from release.gd). Renders SPEC_CAMERAS x SPEC_EXPRESSIONS in the
## proven skin/eye/hair shading pipeline, with neutral studio lighting instead
## of the mood rig, and writes a spec-conformant manifest.json.
## ============================================================================

## Spec convention -> Godot, expressed in the CHARACTER'S OWN measured local
## frame, not a fixed Godot world axis. Both conventions are right-handed (the
## spec says so explicitly), so per-axis this is a pure rotation (no mirror) —
## see the derivation below — but WHICH world direction is "forward" cannot be
## assumed from model_yaw: 272.5deg (this GLB's shipped default) was tuned to
## face a completely unrelated UE-matched camera, not any Godot cardinal axis
## (confirmed empirically: with that yaw, a naive fixed -Z-forward assumption
## rendered a profile view for C1_portrait, not a front view — the tool's own
## DEFAULT_ORBIT_YAW=-72 code comment "lines up with the front-3/4 of the model
## at 272.5" is itself evidence the mesh forward isn't world -Z at that yaw).
## Fix: measure the REAL face-forward direction off the FACIAL_L/R_Eye bones
## via the existing, proven _eye_gaze_frame() (same one "look at camera" uses),
## and build a local (right, up, forward) basis from it.
##
## Derivation (in the local frame): we need T(spec +Z)=up_dir (up->up) and
## T(spec -Y)=fwd_dir (forward->forward). For a proper rotation on an
## orthonormal RH basis with e_x(Godot-canonical)->right_dir, e_y->up_dir,
## e_z->-fwd_dir (Godot's canonical forward is -Z, i.e. canonical +Z is
## "backward"): T(x,y,z) = -x*right_dir + z*up_dir - y*fwd_dir. Verified: spec
## (0,-1,0) [pure forward] -> fwd_dir; spec (0,0,1) [up] -> up_dir. Both check.
func _spec_local_axes() -> Array:   # [right_dir, up_dir, fwd_dir]
	var up_dir := Vector3.UP
	var fwd_dir := Vector3(0, 0, -1)   # fallback if no eye bones resolved
	if _face_eye_skel != null and _eye_bone_l >= 0:
		var fr: Array = _eye_gaze_frame()
		fwd_dir = (fr[1] as Basis).z.normalized()   # Basis(right,up,fwd) per _eye_gaze_frame
	else:
		push_warning("[compare] no eye bones resolved -- falling back to world -Z as face-forward (UNVERIFIED for this character)")
	var right_dir := fwd_dir.cross(up_dir).normalized()
	return [right_dir, up_dir, fwd_dir]

func _spec_vec(x: float, y: float, z: float, axes: Array) -> Vector3:
	var right_dir: Vector3 = axes[0]; var up_dir: Vector3 = axes[1]; var fwd_dir: Vector3 = axes[2]
	return (-x * right_dir + z * up_dir - y * fwd_dir) / 100.0

## World position of a named bone on the BODY skeleton (populated by the
## existing, proven _resolve_body_skeleton — same lookup pattern it already
## uses for "pelvis"/"calf_l"/"calf_r"). Falls back when the bone isn't found
## (should not happen for head/neck_01/pelvis on a real metahuman_base_skel,
## but a custom/partial rig shouldn't hard-crash the capture).
func _bone_world(bname: String, fallback: Vector3) -> Vector3:
	if _body_skeleton == null:
		return fallback
	var idx := _body_skeleton.find_bone(bname)
	if idx < 0:
		return fallback
	return _body_skeleton.global_transform * _body_skeleton.get_bone_global_pose(idx).origin

## Neutral environment: BG_COLOR only (no sky/IBL — spec says "no IBL/HDRI"),
## AgX tonemap (this project's own shipped default/only mode — see release.gd
## _setup_world; recorded honestly in the manifest as "agx", not claimed as
## "none"), SSIL/SSAO on (part of what makes this pipeline look the way it
## does — see GODOT-WHY-IT-LOOKS-BEST.md), ambient kept low so the mid-gray
## card + 2-light rig do the work, not an ambient dome.
func _setup_world_neutral() -> void:
	_env = Environment.new()
	_env.background_mode = Environment.BG_COLOR
	_env.background_color = Color(0.5, 0.5, 0.5)   # sRGB ~ #808080, 18% gray card
	_env.ambient_light_source = Environment.AMBIENT_SOURCE_DISABLED
	_env.tonemap_mode = Environment.TONE_MAPPER_AGX
	_env.tonemap_exposure = 1.0
	_env.glow_enabled = false   # spec wants a clean, comparable frame — no bloom halo
	_env.ssao_enabled = true
	_env.ssao_radius = 0.4
	_env.ssao_intensity = 0.7
	_env.ssil_enabled = true
	_env.ssil_radius = 5.0
	_env.ssil_intensity = 1.0
	_env.adjustment_enabled = false   # no extra saturation/contrast grade
	var we := WorldEnvironment.new(); we.environment = _env; add_child(we)

## Flat mid-gray backdrop quad, no gradient (spec: "Background: mid-gray #808080
## (18% card), no gradient" — release.gd's own backdrop is a lit gradient quad,
## intentionally NOT reused here).
func _setup_backdrop_neutral() -> void:
	var quad := QuadMesh.new(); quad.size = Vector2(30, 20)
	var mi := MeshInstance3D.new(); mi.mesh = quad; mi.position = Vector3(0, 1.2, -3.0)
	var m := StandardMaterial3D.new()
...
## Neutral 2-point studio rig: key 45 deg camera-left, fill 20% of key energy
## directly opposite, both uncolored (white) — spec: "no colored rims". Rebuilt
## PER SHOT (see _run_comparison_mode) because "45 deg camera-left" is relative
## to whichever of the 7 cameras is currently active, not a single world-fixed
## position. NOTE (recorded honestly, not silently): "3000 lux" in the spec is
## a physical-units target; this project's proven pipeline (release.gd,
## match_ue.gd) uses Light3D.light_energy as a non-physical relative scalar
## throughout (see match_ue.gd's own comment: "UE candela doesn't map to Godot
## energy") — physical light units were NOT enabled here (an untested change
## to a working renderer setup, out of scope tonight). We instead match the
## spec's key:fill RATIO (3000:600 = 5:1) at a light_energy scale visually
## tuned to a correct, non-blown-out exposure. See manifest "lighting_note".
const NEUTRAL_KEY_ENERGY := 2.2
const NEUTRAL_FILL_RATIO := 0.20   # spec: fill = 20% of key

func _setup_lights_neutral_for_camera(cam_pos: Vector3, look_at_pt: Vector3) -> void:
	for k in ["key", "fill"]:
		if _lights.has(k) and is_instance_valid(_lights[k]):
			_lights[k].queue_free()
	var cam_fwd := (look_at_pt - cam_pos)
	if cam_fwd.length() < 0.0001: cam_fwd = Vector3(0, 0, -1)
	cam_fwd = cam_fwd.normalized()
	var horiz := Vector3(cam_fwd.x, 0.0, cam_fwd.z)
	if horiz.length() < 0.0001: horiz = Vector3(0, 0, -1)
	horiz = horiz.normalized()
	# +45 deg around UP from the camera's forward = swings toward the camera's
	# LEFT as seen from behind the camera looking at the subject (verified by
	# rendering C1 and checking which cheek is lit — see STATUS.md).
	var key_dir := horiz.rotated(Vector3.UP, deg_to_rad(45.0))
	var key_pos := look_at_pt - key_dir * 1.6 + Vector3.UP * 0.55
	var key := SpotLight3D.new(); key.name = "SpecKey"
	key.light_color = Color(1, 1, 1)
	key.light_energy = NEUTRAL_KEY_ENERGY
	key.spot_angle = 45.0; key.spot_angle_attenuation = 0.5
	key.spot_range = 12.0; key.spot_attenuation = 0.9
	key.shadow_enabled = true; key.shadow_bias = 0.02; key.shadow_normal_bias = 0.25
	key.shadow_blur = 1.2; key.light_specular = 0.5
	key.look_at_from_position(key_pos, look_at_pt, _stable_up((look_at_pt - key_pos).normalized()))
	add_child(key); _lights["key"] = key
	# Fill: directly opposite (camera-right), same height, no shadow (spec's
	# "fill 20%" is the soft/no-shadow light in a standard 2-point kit).
	var fill_dir := horiz.rotated(Vector3.UP, deg_to_rad(-45.0))
	var fill_pos := look_at_pt - fill_dir * 1.6 + Vector3.UP * 0.55
	var fill := SpotLight3D.new(); fill.name = "SpecFill"
	fill.light_color = Color(1, 1, 1)
	fill.light_energy = NEUTRAL_KEY_ENERGY * NEUTRAL_FILL_RATIO
	fill.spot_angle = 50.0; fill.spot_angle_attenuation = 0.5
	fill.spot_range = 12.0; fill.spot_attenuation = 0.9
	fill.shadow_enabled = false; fill.light_specular = 0.3
	fill.look_at_from_position(fill_pos, look_at_pt, _stable_up((look_at_pt - fill_pos).normalized()))
	add_child(fill); _lights["fill"] = fill

## Drive a bone-rigged eye pair toward a world focal point that is to the
## SUBJECT'S left (conjugate gaze = eyeLookOutLeft + eyeLookInRight together,
## per the spec's own control list for E5). Reuses the proven _aim_eye
## minimal-rotation solver. If the character has no eye bones (boneless/faked
## gaze path), falls back to the iris_offset shader param the tool already
## uses for that case.
func _drive_look_left() -> void:
	if _face_eye_skel != null and _eye_bone_l >= 0:
		var fr: Array = _eye_gaze_frame()
		var origin: Vector3 = fr[0]; var basis: Basis = fr[1]
		var target: Vector3 = origin + basis * Vector3(-0.4, 0.0, 0.6)
		_aim_eye(_eye_bone_l, target)
		_aim_eye(_eye_bone_r, target)
	elif not _eye_mats.is_empty():
		for m in _eye_mats:
			m.set_shader_parameter("iris_offset", Vector2(0.06, 0.0))

func _reset_gaze_straight() -> void:
	if _face_eye_skel != null and _eye_bone_l >= 0:
		var fr: Array = _eye_gaze_frame()
		var origin: Vector3 = fr[0]; var basis: Basis = fr[1]
		_aim_eye(_eye_bone_l, origin + basis * Vector3(0, 0, 20.0))
		_aim_eye(_eye_bone_r, origin + basis * Vector3(0, 0, 20.0))
	elif not _eye_mats.is_empty():
		for m in _eye_mats:
			m.set_shader_parameter("iris_offset", Vector2.ZERO)

func _run_comparison_mode() -> void:
	_char_key = OS.get_environment("COMPARE_CHAR") if OS.has_environment("COMPARE_CHAR") else "her"
	if not PROFILES.has(_char_key): _char_key = "her"
	_profile = PROFILES[_char_key]
	...
	_setup_world_neutral()
	_setup_backdrop_neutral()
	_camera = Camera3D.new(); _camera.name = "Camera3D"
	_camera.near = 0.05; _camera.far = 100.0
	_camera.keep_aspect = Camera3D.KEEP_HEIGHT   # spec FOVs are given as vertical
	_cam_attrs = CameraAttributesPractical.new()
	_cam_attrs.dof_blur_far_enabled = false; _cam_attrs.dof_blur_near_enabled = false
	_camera.attributes = _cam_attrs
	_camera.current = true; add_child(_camera)

	_load_character()   # proven loader: scale-to-1.74m, feet-to-floor, wire materials, skeletons
	# Default 272.5: the tool's OWN calibrated bind-pose-facing constant (p.model_yaw's
	# shipped default, same value match_ue.gd's docstring calls "Sam's tuned frame-0
	# match"). The raw GLB's bind pose does not face any canonical axis on its own: this
	# is the empirically-found yaw that faces the character toward the release-tool's
	# fixed camera. First attempt at COMPARE_YAW=0 (assuming bind pose already faced
	# Godot -Z) produced a profile/back view for C1_portrait -- confirms 0 was wrong.
	var yaw := _envf("COMPARE_YAW", 272.5)
	p.model_yaw = yaw
	_character.rotation.y = deg_to_rad(yaw)
	# _load_character() resolves _body_skeleton AFTER wiring, so bone lookups are valid now.

	var head_w := _bone_world("head", _head_world)
	var neck_w := _bone_world("neck_01", head_w)
	var pelvis_w := _bone_world("pelvis", Vector3(0.0, 0.0, 0.0))
	# Computed ONCE, before any expression/gaze changes (eye bones are still at rest
	# here), and reused for every camera below -- head-relative framing shouldn't
	# move per-expression even though the eyes themselves do for E5.
	var axes := _spec_local_axes()
	var fwd_dir: Vector3 = axes[2]; var up_dir: Vector3 = axes[1]

	# CAMERA POSITIONS stay exactly the spec's head-joint-relative numbers (unchanged
	# below) -- but the "eye midpoint"/"mouth center" LOOK-AT targets are the spec's own
	# qualitative labels, not numeric coordinates, so this lane must operationalize them.
	# First attempt used "head_w +2cm/-4cm" (mirroring the camera table's own small
	# offsets) and it was WRONG: at C3/C4's close range (18cm) and narrow FOV (18deg,
	# ~5.7cm of vertical frame at that distance), a few cm of anatomical error is the
	# ENTIRE frame -- the render landed under the chin, not on the eyes (this did NOT
	# show up on the wide shots, where the same absolute error is a small fraction of a
	# much taller frame). Fix: use the REAL measured eye-bone midpoint (same
	# _eye_gaze_frame() the tool's own "look at camera" feature uses) for eye_mid, and a
	# documented anatomical approximation (eye-to-mouth ~6.5cm, a commonly cited adult
	# stomion offset -- HYPOTHESIS-grade, not measured) dropped from it for "mouth",
	# since no mouth bone exists to measure directly.
	var eye_mid_w := head_w
	if _face_eye_skel != null and _eye_bone_l >= 0:
		var efr: Array = _eye_gaze_frame()
		eye_mid_w = efr[0]
	var mouth_w := eye_mid_w - up_dir * 0.065

	var out_dir: String = OS.get_environment("COMPARE_OUT") if OS.has_environment("COMPARE_OUT") \
		else _outdir().path_join("comparison")
	DirAccess.make_dir_recursive_absolute(out_dir)

	print("[compare] char=%s head_w=%s neck_w=%s pelvis_w=%s eye_mid_w=%s fwd=%s right=%s out=%s" % [_char_key, head_w, neck_w, pelvis_w, eye_mid_w, fwd_dir, axes[0], out_dir])

	var renders := []
	var eyeLook_is_blendshape := _bs_map.has("eyeLookOutLeft")
	for expr in SPEC_EXPRESSIONS:
		_zero_all_blendshapes()
		_reset_gaze_straight()
		var shapes: Dictionary = expr["shapes"]
		for k in shapes.keys():
			_set_blendshape(k, shapes[k])
		var is_look_expr: bool = (String(expr["id"]) == "E5_lookLeft")
		if is_look_expr:
			_drive_look_left()
		# settle: let skinning/shader state catch up before sampling frames
		await get_tree().process_frame
		await get_tree().process_frame
		for cam in SPEC_CAMERAS:
			var pos_cm: Array = cam["pos"]
			var origin_pt: Vector3
			match String(cam.get("origin", "head")):
				"eye_mid": origin_pt = eye_mid_w
				"mouth":   origin_pt = mouth_w
				_:         origin_pt = head_w
			var cam_pos: Vector3 = origin_pt + _spec_vec(pos_cm[0], pos_cm[1], pos_cm[2], axes)
			var look_at_pt: Vector3
			match String(cam["lookat"]):
				"eye_mid": look_at_pt = eye_mid_w
				"mouth":   look_at_pt = mouth_w
				"neck_01": look_at_pt = neck_w
				"pelvis":  look_at_pt = pelvis_w
				_:         look_at_pt = head_w
			_camera.fov = cam["fov"]
			var fwd := (look_at_pt - cam_pos)
			if fwd.length() < 0.0001: fwd = Vector3(0, 0, -1)
			_camera.look_at_from_position(cam_pos, look_at_pt, _stable_up(fwd.normalized()))
			_setup_lights_neutral_for_camera(cam_pos, look_at_pt)
			await RenderingServer.frame_post_draw
			await RenderingServer.frame_post_draw
			var img := get_viewport().get_texture().get_image()
			var fname := "%s_%s.png" % [cam["id"], expr["id"]]
			img.save_png(out_dir.path_join(fname))
			renders.append({"camera": cam["id"], "expression": expr["id"], "file": fname})
			print("[compare] captured ", fname)

	_zero_all_blendshapes()
	_write_comparison_manifest(out_dir, renders, head_w, eyeLook_is_blendshape, axes, eye_mid_w)
	print("[compare] DONE -> ", out_dir)
	await get_tree().create_timer(0.15).timeout
	get_tree().quit()

func _write_comparison_manifest(out_dir: String, renders: Array, head_w: Vector3, eyelook_is_blendshape: bool, axes: Array, eye_mid_w: Vector3) -> void:
	var char_map := {"her": "mh_explainer", "guy": "mh_guy_unmapped"}
	var manifest := {
		"platform": "godot",
		"character": char_map.get(_char_key, _char_key),
		"source_dna_sha256": "unknown — see hand_edits note (asset provenance not independently verified by this lane)",
		"pipeline_commit": _git_rev(),
		"renders": renders,
		"tonemapper": "agx",
		"color_space": "srgb",
		"renderer_version": "Godot " + Engine.get_version_info().get("string", "unknown") + " Forward+",
		"hand_edits": [],
		"godot_char_key": _char_key,
		"godot_glb": _profile.get("glb", ""),
		"camera_convention_note": "Spec cameras converted via a pure rotation (both spec and Godot are right-handed - see _spec_vec/_spec_local_axes in comparison_capture.gd), expressed in the CHARACTER'S OWN measured local frame (right/up/forward), NOT a fixed Godot world axis - model_yaw=272.5 (this GLB's shipped default) does not align to any Godot cardinal axis, it was tuned to face an unrelated UE-matched camera; a fixed -Z-forward assumption was tried first and produced a profile view for C1_portrait instead of a front view. Forward measured off the FACIAL_L/R_Eye bones' rest orientation (same mechanism the tool's own 'look at camera' feature uses). Camera position always relative to the BODY skeleton's real 'head' bone world position (not an AABB approximation). 'eye_mid' lookat = the REAL measured eye-bone midpoint (FACIAL_L_Eye/FACIAL_R_Eye average, via the tool's own _eye_gaze_frame()) -- a head_w+2cm approximation was tried first and, at C3's close 18cm/18deg framing (~5.7cm of vertical frame at that distance), landed under the chin instead of on the eyes; a few cm of anatomical error that a wide shot absorbs is the WHOLE frame up close. 'mouth' lookat = eye_mid dropped 6.5cm along -up (a commonly cited adult eye-to-mouth/stomion offset -- HYPOTHESIS-grade approximation, no mouth bone exists to measure directly, flagging for a lane with ground-truth face landmarks to correct if available). Measured axes this render: forward=%s right=%s eye_mid=%s (world space, meters)." % [str(axes[2]), str(axes[0]), str(eye_mid_w)],
		"lighting_note": "Key 45deg camera-left / fill 20% of key, both white, no IBL, mid-gray (0.5,0.5,0.5) unshaded backdrop, rebuilt per-shot relative to each camera. Godot light_energy is a NON-PHYSICAL relative scalar in this pipeline (matches release.gd/match_ue.gd precedent) - NOT calibrated to the spec's literal 3000 lux; the 5:1 key:fill RATIO is honored, the absolute lux value is not.",
		"camera_origin_deviation": "C3_eye_macro and C4_mouth_macro use the SPEC'S OWN pos/fov numbers unchanged, but measure that offset from the real eye-bone-midpoint / approximated-mouth anchor instead of from the head-joint bone the table header literally specifies. Reason: this skeleton's head bone sits well back/below the eyes (~1.55m vs a ~1.65-1.70m crown estimate on a 1.74m figure) -- absorbed fine by the wide shots (tens of cm of frame) but not by these two (~5.7cm of vertical frame at 18cm/18deg) -- head-relative placement clipped into face geometry (verified by rendering it that way first: blurred skin, no eye/mouth visible). All other cameras (C1/C2/C5/C6/C7) use the literal head-joint-relative reading. Recorded here per the brief's own instruction to record camera deviations rather than silently substitute.",
		"expression_note": "E5_lookLeft: eyeLookOutLeft/eyeLookInRight blendshapes are NOT present on this character's baked shape set (present=%s) - confirmed the pipeline's own documented 'horror-eye fix' (ARKit eyeLook* shapes stripped at bake time). Gaze instead driven by rotating the FACIAL_L_Eye/FACIAL_R_Eye bones (or, for a boneless face, the eye shader's iris_offset param) toward a world point to the subject's left." % eyelook_is_blendshape,
		"head_world_m": {"x": head_w.x, "y": head_w.y, "z": head_w.z},
		"model_yaw_deg": p.model_yaw,
	}
...
func _git_rev() -> String:
	var out := []
	var code := OS.execute("git", ["-C", ProjectSettings.globalize_path("res://"), "rev-parse", "HEAD"], out, true)
